...
def project (input): # kigyűjti az előforduló projekteket és a gyakoriságukat
    projects = dict()
    for i in range(1,len(input)): 
        id = input[i][4]
        if id not in projects: projects[id]=1
        else: projects[id]+=1
...
def sizes(input, project_id): # adott projektben adott hosszúságú és szélességű kelyhek előfordulása
    ...
    with p.open('w') as file:
        file.write(str(output))  # length, width, gyakorisag   
    return output

def time(input, project_id): # gyártáshoz szükséges idő
    date = []
    for i in range(1,len(input)):
        if input[i][4] == project_id:
